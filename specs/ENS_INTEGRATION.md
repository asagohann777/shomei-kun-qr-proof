...
Changing an ENS address changes future searches and future issuance. It never rewrites an existing card. No card subnames, new ENS Registry/Resolver deployment or persistent search database are used.
...
## Optional recipient issuance

Set `ENS_SEPOLIA_RPC_URL` in the issuer environment. Use `--recipient-ens shomeikun.eth` instead of `--wallet ADDRESS`. The CLI displays the normalized name, resolved address and issuance chain, requires confirmation, checks that the address has no contract code on both chains, and resolves again immediately before signing. A changed address stops issuance.

The state file stores the name, Sepolia chain ID and frozen address. `issuer resume --state FILE` uses the saved signed transaction without looking up ENS again. Old state files remain compatible. `--wallet ADDRESS` remains supported; omitting both recipient options issues an unrestricted card. The options are mutually exclusive.
...
The user subsequently reported that the iPhone app crashed during the test and displayed "結果を確認できません" after reopening. The public card API still returned `unregistered` with no registration evidence when checked after the report. This does not rule out a pending transaction. The point at which the app crashed and the MetaMask transaction history have not yet been confirmed. Recovery diagnosis remains open; physical iPhone verification has not passed. PC Chrome + MetaMask was confirmed by the user. Prize eligibility is not established solely by these checks.
