...
UI開発と独立したAPIでCurvegrid TestnetとMultiBaasへ接続し、任意の本人ウォレットによるカード所有者登録と、その記録の再確認を行う。既存モックの互換性を維持し、実環境の成功と未検証の状態を区別する。

## ADDED Requirements

### Requirement: 設計承認と担当分離
実装担当はユーザーの明示的な詳細設計承認を受けるまで、実装コード・依存関係・デプロイ設定を変更してはならない（MUST NOT）。担当境界はユーザーの最新指示に従わなければならない（MUST）。2026-09-26の追加指示で画面デザインはUI担当、API接続・ブラウザ署名はこのPRの実装担当とする。
...
コントラクトは発行済み・未登録・接続ウォレット本人の条件を満たす取引だけを登録しなければならない（MUST）。ownerとnicknameを同一取引に結び付け、再登録・移転・取消し・上書きを提供してはならない（MUST NOT）。
