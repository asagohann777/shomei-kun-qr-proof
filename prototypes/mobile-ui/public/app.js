...
function liveCameraView() {
  ...
  return `<section class="page camera-page live-camera-page">${active && cameraState.torch ? `<button class="flash-button" data-action="flash" aria-pressed="${cameraState.light}">${icon('flash')}<span>${t('flash')}</span></button>` : ''}<div class="scanner live-scanner" aria-label="${t('scanHint')}"><span id="camera-slot"></span><div class="scan-corners" aria-hidden="true"><span></span><span></span><span></span><span></span></div>${active ? '<div class="scan-light-track" aria-hidden="true"><div class="scan-sweep"></div></div>' : ''}</div><h1 class="scan-status" role="status">${escape(t(label))}</h1><div class="scan-tools"><button class="round-tool" data-action="photos" ${cameraState.kind === 'photo' ? 'disabled' : ''}>${icon('photo')}<span>${t('fromPhotos')}</span></button><button class="round-tool" data-action="help">${icon('info')}<span>${t('help')}</span></button></div></section>${!active ? footer('resume-camera', t(cameraState.kind === 'paused' ? 'cameraResume' : 'cameraRetry'), '', busy) : ''}`;
}
