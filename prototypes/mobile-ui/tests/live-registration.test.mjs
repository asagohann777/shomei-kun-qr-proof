...
function fixture(o = {}) {
 ...
 const wallet = { connect: async () => owner, snapshot: async () => owner, switchChain: async () => {}, disconnect: async () => {}, dispose() {}, send: async () => { sent++; if(o.send) return o.send(); card = { ...card, status: 'registered', owner: { address: account, nickname }, evidence: { status: 'available', transactionHash: hash, blockNumber: 2 } }; return hash; } };
...
test('connection controls do not overwrite uncertain registration',async()=>{const f=fixture({send:async()=>{throw Error('lost');}});await ready(f);await f.controller.register(nickname);await f.controller.connect();await f.controller.switchChain();assert.equal(f.controller.getSnapshot().registration.kind,'unknown');});
