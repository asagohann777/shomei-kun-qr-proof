export function uiConfig(env) {
  ...
  if (apiMode === 'mock') return { apiMode, walletMode, publicUrl, cardId };
  if (!env.UI_API_BASE_URL) throw new Error('UI_API_BASE_URL is required for live API');
  ...
  return { apiMode, walletMode, apiBaseUrl: api.origin, publicUrl, cardId, ...(rpcOrigin ? { rpcOrigin } : {}) };
}
